  ...
  public void testSimpleCountWithTranspose() {
    ...
    String expectedLogical =
        "LogicalProject(column=[$0], row 1=[$1], row 2=[$2], row 3=[$3], row 4=[$4], row 5=[$5])\n"
            + "  LogicalAggregate(group=[{0}], row 1_null=[MAX($1) FILTER $2], row 2_null=[MAX($1)"
            + " FILTER $3], row 3_null=[MAX($1) FILTER $4], row 4_null=[MAX($1) FILTER $5], row"
            + " 5_null=[MAX($1) FILTER $6])\n"
            + "    LogicalProject(column=[$2], value=[CAST($3):VARCHAR NOT NULL], $f4=[=($1, 1)],"
            + " $f5=[=($1, 2)], $f6=[=($1, 3)], $f7=[=($1, 4)], $f8=[=($1, 5)])\n"
    ...
    String expectedSparkSql =
        "SELECT `column`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` = 1)"
            + " `row 1`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` = 2)"
            + " `row 2`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` = 3)"
            + " `row 3`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` = 4)"
            + " `row 4`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` = 5)"
            + " `row 5`\n"
  ...
  public void testMultipleAggregatesWithAliasesTranspose() {
    ...
    String expectedLogical =
        "LogicalProject(column=[$0], row 1=[$1], row 2=[$2], row 3=[$3], row 4=[$4], row 5=[$5])\n"
            ...
            + " FILTER $3], row 3_null=[MAX($1) FILTER $4], row 4_null=[MAX($1) FILTER $5], row"
            + " 5_null=[MAX($1) FILTER $6])\n"
            + "    LogicalProject(column=[$5], value=[CAST($6):VARCHAR NOT NULL], $f7=[=($4, 1)],"
            + " $f8=[=($4, 2)], $f9=[=($4, 3)], $f10=[=($4, 4)], $f11=[=($4, 5)])\n"
  ...
  public void testTransposeWithLimit() {
    ...
    String expectedLogical =
        "LogicalProject(column=[$0], row 1=[$1], row 2=[$2], row 3=[$3])\n"
            + "  LogicalAggregate(group=[{0}], row 1_null=[MAX($1) FILTER $2], row 2_null=[MAX($1)"
            + " FILTER $3], row 3_null=[MAX($1) FILTER $4])\n"
            + "    LogicalProject(column=[$5], value=[CAST($6):VARCHAR NOT NULL], $f7=[=($4, 1)],"
            + " $f8=[=($4, 2)], $f9=[=($4, 3)])\n"
            + "      LogicalFilter(condition=[IS NOT NULL($6)])\n"
  ...
  public void testTransposeWithLimitColumnName() {
    ...
    String expectedLogical =
        "LogicalProject(column_names=[$0], row 1=[$1], row 2=[$2], row 3=[$3])\n"
            + "  LogicalAggregate(group=[{0}], row 1_null=[MAX($1) FILTER $2], row 2_null=[MAX($1)"
            + " FILTER $3], row 3_null=[MAX($1) FILTER $4])\n"
            + "    LogicalProject(column_names=[$5], value=[CAST($6):VARCHAR NOT NULL], $f7=[=($4,"
            + " 1)], $f8=[=($4, 2)], $f9=[=($4, 3)])\n"
            + "      LogicalFilter(condition=[IS NOT NULL($6)])\n"
            + "        LogicalProject(ENAME=[$0], COMM=[$1], JOB=[$2], SAL=[$3],"
    ...
    String expectedSparkSql =
        "SELECT `column_names`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_`"
            + " = 1) `row 1`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` ="
            + " 2) `row 2`, MAX(CAST(`value` AS STRING)) FILTER (WHERE `_row_number_transpose_` ="
            + " 3) `row 3`\n"
            + "FROM (SELECT `t`.`ENAME`, `t`.`COMM`, `t`.`JOB`, `t`.`SAL`,"
