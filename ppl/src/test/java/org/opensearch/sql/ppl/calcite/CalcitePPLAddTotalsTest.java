...
public class CalcitePPLAddTotalsTest extends CalcitePPLAbstractTest {
  ...
  public void testAddTotalsWithAllOptionsIncludingFieldname() throws IOException {
    String ppl =
        "source=EMP  | fields DEPTNO, SAL, JOB | addtotals SAL label='GrandTotal'"
    ...
    String expectedLogical =
        "LogicalUnion(all=[true])\n"
            ...
            + "    LogicalTableScan(table=[[scott, EMP]])\n"
            + "  LogicalProject(DEPTNO=[null:NULL], SAL=[$0], JOB=[null:NULL],"
            + " CustomSum=[null:NULL], all_emp_total=['GrandTotal'])\n"
            + "    LogicalAggregate(group=[{}], SAL=[SUM($0)])\n"
            + "      LogicalProject(SAL=[$5])\n"
            + "        LogicalTableScan(table=[[scott, EMP]])\n";
    verifyLogical(root, expectedLogical);
    ...
    String expectedSparkSql =
        "SELECT `DEPTNO`, `SAL`, `JOB`, `SAL` `CustomSum`, NULL `all_emp_total`\n"
            + "FROM `scott`.`EMP`\n"
            + "UNION ALL\n"
            + "SELECT NULL `DEPTNO`, SUM(`SAL`) `SAL`, NULL `JOB`, NULL `CustomSum`, 'GrandTotal'"
            + " `all_emp_total`\n"
            + "FROM `scott`.`EMP`";
    verifyPPLToSparkSQL(root, expectedSparkSql);
  }
  ...
  public void testAddTotalsMatchingLabelFieldWithExisting() throws IOException {
    String ppl =
        "source=EMP  | fields DEPTNO, SAL, JOB | addtotals SAL DEPTNO col=true label='GrandTotal'"
            + " labelfield='JOB' ";
