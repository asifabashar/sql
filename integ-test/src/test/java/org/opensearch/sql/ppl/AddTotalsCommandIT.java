...
public class AddTotalsCommandIT extends PPLIntegTestCase {
    ...
    public void compareDataRowTotals(org.json.JSONArray dataRows, List<Integer> field_indexes, int totalColIndex) {
        for (int i = 0; i < dataRows.length(); i++) {
            var row = dataRows.getJSONArray(i);
    ...
    public void verifyColTotals(org.json.JSONArray dataRows,List<Integer> field_indexes, String finalSummaryEventLevel) {
    ...
    public void testAddTotalsRowFieldsNonNumeric() throws IOException {
        ...
        // Verify that we get original rows plus totals row
        verifySchema(
                result, schema("age", "bigint"), schema("address", "string"), schema("balance", "bigint"), schema("Total", "bigint"));
    ...
    public void testAddTotalsWithCustomLabel() throws IOException {
        var result =
                executeQuery(
                        String.format(
                                "source=%s | where age > 25 | head 2|fields age, balance | addtotals fieldname='Grand Total'",
                                TEST_INDEX_ACCOUNT));

        verifySchema(
                result, schema("age", "bigint"), schema("balance", "bigint"), schema("Grand Total", "bigint"));
    ...
    public void testAddTotalsWithLabelAndLabelField() throws IOException {
        var result =
                executeQuery(
                        String.format(
                                "source=%s | where age > 25 |head 3| fields age, balance | addtotals row=false col=true label='Sum'  labelfield='Total Summary'",
                                TEST_INDEX_ACCOUNT));

        // Verify schema includes custom fieldname
        verifySchema(
                result, schema("age", "bigint"), schema("balance", "bigint"), schema("Total Summary", "string"));
