...
public class AddColTotalsCommandIT extends PPLIntegTestCase {
    ...
    public void testAddColTotalsWithCustomLabel() throws IOException {
        var result =
                executeQuery(
                        String.format(
                                "source=%s | where age > 25 | head 2|fields age, balance | addcoltotals label='Sum' labelfield='Grand Total'",
                                TEST_INDEX_ACCOUNT));

        verifySchema(
                result, schema("age", "bigint"), schema("balance", "bigint"), schema("Grand Total", "string"));
    ...
    public void testAddColTotalsWithLabelAndLabelField() throws IOException {
        var result =
                executeQuery(
                        String.format(
                                "source=%s | where age > 25 |head 3| fields age, balance,firstname | addcoltotals  age balance  label='Sum'  labelfield='firstname'",
                                TEST_INDEX_ACCOUNT));

        // Verify schema includes custom fieldname
        verifySchema(
                result, schema("age", "bigint"), schema("balance", "bigint"), schema("firstname", "string"));
